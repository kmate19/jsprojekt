...
function pontok(db){
    var k=0
    for (let i = 0; i < 4; i++) {
        var pontsordiv = document.createElement("div")
        pontsordiv.classList += " pontsordiv"
        for (let j = 0; j < db; j++) 
        {
            var pontoszlopdiv = document.createElement("div")
            pontoszlopdiv.classList += " oszlopdiv"
            pontoszlopdiv.id=k
            k++
            pontsordiv.appendChild(pontoszlopdiv)
        }
        tabla.appendChild(pontsordiv)
    }
}
