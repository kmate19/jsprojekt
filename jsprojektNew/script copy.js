...
function Kever() {
    let segedt=new Array()
    for (let index = 0; index < 30; index++) {
        let sv=randomsz(30,0)
        while (segedt.includes(sv)) {
            sv=randomsz(30,0)
        }
...
function valuesSor() {
    let ids=new Array();
    let sorvalue=0;
    console.log("sorok osszege: ");
    for (let index = 0; index < 30; index++) {
        currentdiv=document.getElementById(index)   
        id=currentdiv.firstChild.id.replace('k','')
        ids.push(id)
    }
    for (let index = 0; index < 30; index++) {
        currentkartya=t[ids[index]]
        currentkartyavalue=currentkartya.info.value
        sorvalue+=currentkartyavalue
        if ((index+1)%6==0) {
...
function valuesOszlop() {
    let ids=new Array();
    let sorvalue=0;
    console.log("oszlopok osszege: ");
    for (let findex = 0; findex < 6; findex++) {
        for (let index = 0; index < 5; index++) {
            currentdiv=document.getElementById(index*6+findex)   
            id=currentdiv.firstChild.id.replace('k','')
            ids.push(id)
        }
    }
